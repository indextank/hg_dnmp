...
if [ -z "${EXTENSIONS##*,gd,*}" ]; then
    echo "---------- Install gd ----------"
    apk update \
    && apk add --no-cache tini libintl freetype freetype-dev jpeg jpeg-dev libjpeg libjpeg-turbo libjpeg-turbo-dev libpng-dev gmp gmp-dev libzip-dev libssh2-dev libwebp-dev libxpm-dev libvpx-dev
    && docker-php-ext-configure gd --with-freetype=/usr/include/ --with-xpm=/usr/include/ --with-webp=/usr/include/ --with-jpeg=/usr/include/ \
