...
echo
echo "============================================"
echo "Install extensions from   : php72-debian.sh"
...
if [ "${PHP_EXTENSIONS}" != "" ]; then
    ...
    apt-get update \
    && apt-get install -y gcc g++ make cmake autoconf automake libtool libssl-dev
fi

# Install gd for Webp plugin
if [ ! -f libwebp-${LIBWEBP_VERSION}.tar.gz ]; then
    apt-get install wget \
    && wget --no-check-certificate https://sources.voidlinux.org/libwebp-${LIBWEBP_VERSION}/libwebp-${LIBWEBP_VERSION}.tar.gz
fi
